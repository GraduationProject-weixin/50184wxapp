...
此次开发这个“村游网”系统的微信小程序，不仅仅满足用户的需要，也能跟上时代的发展风向，从技术的角度还是用户的角度上进行开发都是很有意义的。
## 1.4 论文研究内容
...
（3）景点分类实体属性图通过Visio工具绘制，绘制结果展示如下：

![](/md/blog.008.png)

图4-6 景点分类实体属性图
### 4.3.2 数据库表结构
在进行这部分设计之前，需要明白和掌握数据类型以及各个数据类型的长度范围等知识，因为在一张具体的数据表中，为了方便理解，这里就举个简单的例子。比如用户信息表，这个表格的字段就是用户这个实体具备的属性，这时就需要对字段进行数据类型，以及字段长度的设置，也要设置一个主键来作为用户信息表的唯一标识。这些都是数据库表结构设计需要完成的内容。根据“村游网”系统的微信小程序的功能设计以及数据库设计要求，展示该系统的数据表结构。 
...
|6|dengji|String|等级|是|
|7|jingdiantupian|String|景点图片|是|
|8|jiage|Integer|价格|是|
|9|kaifangshijian|String|开放时间|是|
|10|fuwudianhua|String|服务电话|是|
|11|xiangxidizhi|String|详细地址|是|
|12|jingdianjieshao|String|景点介绍|是|
4.4收藏表表
...
|4|jingdianmingcheng|String|景点名称|是|
|5|jingdianfenlei|String|景点分类|是|
|6|jingdiantupian|String|景点图片|是|
|7|jiage|Integer|价格|是|
|8|kaifangshijian|String|开放时间|是|
|9|fuwudianhua|String|服务电话|是|
|10|xiangxidizhi|String|详细地址|是|
|11|jingdianjieshao|String|景点介绍|是|
...
|1|Id|Int|id|否|
|2|addtime|Date|创建时间|是|
|3|meishibianhao|String|美食编号|是|
|4|meishimingcheng|String|美食名称|是|
|5|meishileixing|String|美食类型|是|
|6|tupian|String|图片|是|
|7|chandi|String|产地|是|
|8|shengchanriqi|datetime|生产日期|是|
|9|meishijianjie|String|美食简介|是|
|10|price|float|价格|是|
4.12在线客服表
